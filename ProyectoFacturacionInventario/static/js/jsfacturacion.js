...
var vents = {  
    ...
    calcular_factura: function () {
        ...
        dict.varsubtotal =  dict.cant_facturar * parseFloat(dict.detalle_producto.precio_venta)- parseFloat(dict.desc_producto);
...
$(function () {   //Funcion para buscar los productos y traerlos en formato json al html  utilizando autocomplete de las librerias JQuery ui 
    ...
    //funcion para buscar personas y traerlos a formato json al html asi mismo para poder pintarlos en el HTML 
    $('input[name="search2"]').autocomplete({
        ...
        select: function (event, ui) {
            event.preventDefault();
            console.clear();
            vents.items.cliente.push(ui.item);
            console.clear();
            console.log(vents.items);
            $(this).val('');

        // a parti de aqui incia el proceso de pintar los datos que ya estan traidos con vents.items.cliente.push(iu.item) 
        let res=document.querySelector('#tblpersons');  
        res.innerHTML='';
        for(let i of vents.items.cliente){
            res.innerHTML =`
       <tr>
            <td><b>Nombre cliente: </b>${i.nombre_persona}  ${i.apellido_persona}</td>   
        </tr>        
        <tr>
            <td><b>Direccion: </b>${i.direccion_persona}</td>   
        </tr>
        <tr>
            <td><b>Nit: </b>${i.nit_persona}</td>   
        </tr>`
         }
...
$('form').on('submit',function (e) {
    ...
    parameters.append('vents', JSON.stringify(vents.items));  // convierte diccionario a tipo string
